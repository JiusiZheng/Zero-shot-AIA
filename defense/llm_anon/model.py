"""LLM loading and batched generation (single-model use only)."""
...
from config import HF_CACHE_DIR  # noqa: E402
...
def load_llm(model_id: str):
    tokenizer_cls, model_cls = _hf_classes()
    eprint(f"[llm] loading {model_id} ...")
    tokenizer = tokenizer_cls.from_pretrained(model_id, cache_dir=HF_CACHE_DIR, trust_remote_code=True)
...
def generate_batch(tokenizer, model, user_contents: list[str], max_new_tokens: int, temperature: float, batch_size: int) -> list[str]:
    ...
    while i < len(user_contents):
        ...
        try:
            ...
            bs = max(1, bs // 2)
            eprint(f"  [warn] CUDA OOM, retrying with batch_size={bs}")
            continue
