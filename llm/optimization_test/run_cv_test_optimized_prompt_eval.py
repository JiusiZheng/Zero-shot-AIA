...
using the trimmed ("optimized") prompts. Test-set counterpart of
../optimization_dev/run_cv_dev_optimized_prompt_eval.py (`import ... as
base`), adding per-subset splitting and mean +/- sample-std aggregation
across CV_test_1/2/3."""
...
OPTIMIZATION_DEV_DIR = PROJECT_DIR / "optimization_dev"
sys.path.insert(0, str(OPTIMIZATION_DEV_DIR))

import run_cv_dev_optimized_prompt_eval as base  # noqa: E402
...
# Full-feature caches already built by ../run_cv_test_attribute_inference.py (one per subset); reused, never modified.
ORIGINAL_TEST_FEATURES_CACHE_DIR = PROJECT_DIR / "results"

MODEL_LLMS = base.MODEL_LLMS
ATTRIBUTE_VALID_LABELS = base.ATTRIBUTE_VALID_LABELS
TABLE_COLUMNS = base.TABLE_COLUMNS
FEATURE_SUBSET_KEYS = base.FEATURE_SUBSET_KEYS
...
def feature_cache_path(subset: str) -> Path:
    return RESULTS_DIR / f"cv_test_optimized_features_cache_{subset}.json"


def original_feature_cache_path(subset: str) -> Path:
    return ORIGINAL_TEST_FEATURES_CACHE_DIR / f"cv_test_features_cache_{subset}.json"


def results_path_for(model_name: str) -> Path:
    return RESULTS_DIR / f"cv_test_optimized_results_{base.slugify(model_name)}.json"
...
# Stage 1: feature cache per subset (CPU only); holds the full, untrimmed
# extractor output -- FEATURE_SUBSET_KEYS trimming happens at prompt-build time.

def load_original_feature_cache(subset: str) -> dict:
    path = original_feature_cache_path(subset)
    if not path.exists():
        return {}
...
def build_feature_cache(subset: str, limit: int | None = None) -> dict:
    """cache[audio_path][attribute] = full features dict. Resumable; reuses
    ../results/cv_test_features_cache_<subset>.json when available."""
    ...
    original_cache = load_original_feature_cache(subset)
...
def run_inference_for_model(model_name: str, attributes: list[str], limit: int | None = None) -> dict:
    ...
    for subset in SUBSETS:
        ...
        for attribute in attributes:
            ...
            prompts = []
            for rec in records:
                full_feats = feature_cache[rec["audio_path"]][attribute]
                prompts.append(base.build_full_prompt(attribute, full_feats))
...
def aggregate_across_subsets(per_subset: dict[str, dict], attributes: list[str]) -> dict:
    ...
    LR-classifier / ALM / original-prompt LLM test-set tables."""
    out = {}
    for attribute in attributes:
...
def aggregate_and_print_table(model_names: list[str]) -> None:
    ...
    out_path = RESULTS_DIR / "cv_test_optimized_attribute_inference_table.txt"
    lines = []
    lines.append("3 CommonVoice test sets (3_testsets_attribute_labels.json, CV_test_1/2/3, n=150 each) -- "
                  "LLM text-only attribute inference, OPTIMIZED (trimmed-feature) prompts")
    lines.append("Prompts: optimization_dev/optimized_prompts/optimized_prompt_<attribute>.txt (verbatim). "
                  "Features: only the subset each optimized prompt names (see FEATURE_SUBSET_KEYS).")
